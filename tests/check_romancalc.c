...
START_TEST(test_romancalc_simple_subtract)
{
  ...
  ck_assert_ptr_ne(calc, NULL);
  ck_assert(romancalc_subtract(calc, "XII"));
  ck_assert(romancalc_value(calc, value, MAX_VALUE_LENGTH));
...
END_TEST
START_TEST(test_romancalc_complex_subtract)
{
  RomanCalc *calc=NULL;
  char *value=(char*)malloc(sizeof(char)*MAX_VALUE_LENGTH);
  ck_assert_ptr_ne(value, NULL);
  calc = romancalc_create("LXXIII");
  ck_assert_ptr_ne(calc, NULL);
  ck_assert(romancalc_subtract(calc, "XLIV"));
  ck_assert(romancalc_value(calc, value, MAX_VALUE_LENGTH));
  ck_assert_str_eq(value, "XXIX");
  free(value);
  value=NULL;
  romancalc_free(calc);
  calc=NULL;
...
Suite * make_romancalc_suite(void)
{
  ...
  tcase_add_test(tc_add, test_romancalc_complex_subtract);
